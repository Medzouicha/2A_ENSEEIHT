...
type info =
  ...
  (*Info pour stocker les etiquettes *)
  | InfoEtiquette of string * int * string
...
let string_of_info info =
  ...
  | InfoFun (n,t,tp) -> "Fonction "^n^" : "^(List.fold_right (fun elt tq -> if tq = "" then (string_of_type elt) else (string_of_type elt)^" * "^tq) tp "" )^
                      " -> "^(string_of_type t)
  |InfoEtiquette(n,dep,base)->"etiquette" ^n^" "^(string_of_int dep)^"["^base^"]"
...
(* Modifie l'emplacement (dépl, registre) si c'est une InfoVar, ne fait rien sinon *)
 let modifier_adresse_variable d b i =
     ...
     |InfoEtiquette(n,_,_) -> i:= InfoEtiquette (n,d,b)
     | _ -> failwith "Appel modifier_adresse_variable pas sur un InfoVar ou InfoEtiquette"
