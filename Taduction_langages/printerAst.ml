...
struct
  ...
  let string_of_binaire b =
   ...
   let rec string_of_affectable a = 
    ...
    | Case (a,e) -> (string_of_affectable a) ^ (string_of_expression e)
  ...
  and string_of_expression e =
    ...
    | Creation (t,e) -> (string_of_type t)^(string_of_expression e)
    | Initialisation le -> ((List.fold_right (fun i tq -> (string_of_expression i)^tq) le ""))
  ...
  let rec string_of_instruction i =
    ...
    | For (t,n1,e1,e2,n2,e3,_) -> "for ( "^(string_of_type t)^n1^(string_of_expression e1)^";"^(string_of_expression e2)^";"^n2^"="^(string_of_expression e3)^")"
    | Goto (n) -> "goto "^n
    | DeclGoto (n) -> "etiq "^n
