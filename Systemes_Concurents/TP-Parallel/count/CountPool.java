...
public class CountPool {
    ...
    private static int computeCount(ExecutorService executor, int[] array, int numberOfTasks) throws InterruptedException, ExecutionException {
        ...
        for (int i = 0; i < numberOfTasks; i++) {
            int end = Math.min(start + taskSize, array.length);
            Future<Integer> result = executor.submit(new PartialCount(array, start, end));
            results.add(result);
            start = end;
        }
        ...
        return totalCount;// pour que ça compile
    }
