...
function regions_de_confiance(f::Function, gradf::Function, hessf::Function, x0::Vector{<:Real};
    ...
    while (nb_iters < max_iter )
        #&& norm(gradf(xk))> max(tol_rel*norm(gradf(x0)), tol_abs) && (norm(gradf(xk))<= max(tol_rel*norm(gradf(x0)), tol_abs)) && (norm(dk) <= epsilon*max(tol_rel*norm(xk), tol_abs)) && (abs(f(xk_1) - f(xk)) < epsilon*max(tol_rel*abs(f(xk)), tol_abs)))
        nb_iters = nb_iters + 1

        if (algo_pas == "cauchy") 
            sk = cauchy(gradf(xk), hessf(xk), Δk)
        elseif (algo_pas == "gct")
            sk = gct(gradf(xk), hessf(xk), Δk)
        end
        function m(s)
            return f(xk) + transpose(gradf(xk))*s + (1/2)*transpose(s)*hessf(xk)*s
        end
        ρk = (f(xk) - f(xk + sk))/(m(zeros(n))-m(sk))
        if  ρk >= η1
            xk_1 = xk + sk
        end
        xs = vcat(xs, [xk_1])
        ...
        elseif (norm(sk) <= epsilon*max(tol_rel*norm(xk), tol_abs))
            flag = 1
            break 
        elseif (abs(f(xk + sk) - f(xk)) < epsilon*max(tol_rel*abs(f(xk)), tol_abs)) #changer xk + sk par xk_1"
            flag = 2 
            break 
